 ...
 * 
 * This class is optimized for the Difference Of Gaussian detector used in David
 * Lowe's SIFT-algorithm \citep{Loew04}.
 * 
 ...
 * Octaves contain overlapping scales of the scalespace. Thus it is possible to
 * execute several operations that depend on adjacent scales within one octave.
 * 
 * BibTeX:
 * 
 * <pre>
...
public class FloatArray2DScaleOctave {
	 ...
	 * that is, you have to take care, that dst.width == src.width / 2 +
	 * src.width % 2 && dst.height == src.height / 2 + src.height % 2 .
	 * 
	 * @param src
	 *            the source image
	 * @param dst
	 ...
	 * 
	 * For efficiency reasons, the dimensions of {@link dst} are not checked,
	 * that is, you have to take care, that src.width == dst.width / 2 +
	 * dst.width % 2 && src.height == dst.height / 2 + dst.height % 2 .
	 * 
	 * @param src
	 *            the source image
	 * @param dst
	...
	/**
	 * 1D gaussian kernels required to create the corresponding gaussian image
	 * instances from the first one
	 */
	 ...
	 * @param img
	 *            image being the first gaussian instance of the scale octave
	 *            img must be a 2d-array of float values in range [0.0f, ...,
	 *            1.0f]
	 * @param initial_sigma
	 *            inital gaussian sigma
	 */
	public FloatArray2DScaleOctave(FloatArray2D img, int steps, float initial_sigma) {
		...
		// System.out.println( "sigma[0] = " + SIGMA[ 0 ] + "; sigma_diff[0] = "
		// + SIGMA_DIFF[ 0 ] );
	 ...
	 * without creating each scale octave. Like proposed by Lowe \citep{Lowe04},
	 * you can use this image to build the next scale octave. Taking every
	 * second pixel of this image, you get a gaussian image with INITIAL_SIGMA
	 * of the half image size.
	 */
