...
public class FloatArray2DSIFT {
	...
	public static float[] featureSizeHistogram(Vector<Feature> features, float min, float max, int bins) {
		...
		for (Feature f : features) {
			int bin = Math.max(0, Math.min(bins - 1, (int) (Math.log(f.scale) / Math.log(2.0) * 28.0f)));
			++hb[bin];
		}
	...
	public FloatArray2DSIFT(int feature_descriptor_size, int feature_descriptor_orientation_bins) {
		...
		FEATURE_DESCRIPTOR_ORIENTATION_BIN_SIZE = 2.0f * (float) Math.PI / FEATURE_DESCRIPTOR_ORIENTATION_BINS;
	...
	/**
	 * sample the scaled and rotated gradients in a region around the features
	 * location, the regions size is defined by ( FEATURE_DESCRIPTOR_SIZE * 4
	 * )^2 ( 4x4 subregions )
	 * 
	 * @param c
	 ...
	 *            octave index
	 * @param octave_sigma
	 *            sigma of the corresponding gaussian kernel with respect to the
	 *            scale octave
	 * @param orientation
	 *            orientation [-&pi; ... &pi;]
	 */
	 ...
	 * @param steps
	 *            gaussian smooth steps steps per scale octave
	 * @param initial_sigma
	 *            sigma of the generating gaussian kernel of img
	 * @param min_size
	 *            minimal size of a scale octave in pixel
	 * @param max_size
	 *            maximal size of an octave to be taken into account Use this to
	 *            save memory and procesing time, if processing higher
	 *            resolutions is not necessary.
	 */
	public void init(FloatArray2D src, int steps, float initial_sigma, int min_size, int max_size) {
		// estimate the number of octaves needed using a simple while loop
		// instead of ld
		int o = 0;
		float w = src.width;
		float h = src.height;
		...
		for (int i = 0; i < octaves.length; ++i) {
			octaves[i] = new FloatArray2DScaleOctave(src, sigma, sigma_diff, kernel_diff);
			octaves[i].buildStub();
			next = new FloatArray2D(src.width / 2 + src.width % 2, src.height / 2 + src.height % 2);
	...
	public Vector<Feature> run() {
		...
		for (int o = 0; o < octaves.length; ++o) {
			if (octaves[o].state == FloatArray2DScaleOctave.State.EMPTY) {
				continue;
			}
	...
	public Vector<Feature> run(int max_size) {
		...
		// " candidates processed in all octaves" );
		return features;
	}
	...
	public Vector<Feature> runOctave(int o) {
		...
		// System.out.println( features.size() +
		// " candidates processed in octave " + o );
